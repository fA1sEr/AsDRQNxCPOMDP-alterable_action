...
class ReplayMemory:
    ...
    def calculate(self):
        ...
        for i in range(self.n_old):
            q1 = np.max(self.network.get_q(self.s2[self.begin+i].reshape(1,26), state_in), axis=1)
            q2 = self.network.get_q(self.s1[self.begin+i].reshape(1,26), state_in)[self.a[self.begin+i]]
